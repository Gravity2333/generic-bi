...
import { IWidgetContextContext, WidgetContext } from '@/layouts/WidgetLayout';
...
const Share = ({ onWidgetReady }: IWidgetPreviewProps) => {
  const { datasets: schemaDetails = [], dicts = [] } = useContext<IGlobalContext>(GlobalContext);
  const { rangeFromOutSystem, pageEmbed } = useContext<IWidgetContextContext>(WidgetContext);
  const time_range = rangeFromOutSystem;
  const time_grain = getMatchedInterval(time_range?.custom[0]!, time_range?.custom[1]!);
